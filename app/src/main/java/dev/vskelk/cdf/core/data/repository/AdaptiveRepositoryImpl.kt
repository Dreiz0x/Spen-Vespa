...
class AdaptiveRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPrioritizedReactivos(
        limit: Int,
        modulo: String,
        examArea: String?
    ): List<ReactivoUI> {
            ...
            ReactivoUI(
                ...
                opciones = opciones.map { op ->
                    OpcionUI(
                        ...
                        distractorTipo = op.distractorTipo
                    )
                },
    ...
    override suspend fun recordAnswer(
        ...
        val intento = ReactivoIntentoEntity(
            ...
            errorType = errorType ?: selectedOption?.distractorTipo?.let {
                ErrorType.fromDistractor(it)
            }
        )
    ...
    override suspend fun completeSession(sessionId: Long): SesionResultado {
        ...
        val weakSubtemas = intentos
            .filter { !it.isCorrect }
            .map { attempt ->
                reactivoDao.getReactivoById(attempt.reactivoId)?.subtemaId
            }
            .filterNotNull()
            .distinct()
        ...
        val mensaje = when {
            precision >= 0.8f -> "¡Excelente dominio del tema!"
            precision >= 0.6f -> "Buen progreso, continúa practicando."
            precision >= 0.4f -> "Hay áreas por fortalecer."
            else -> "Se requiere más estudio en estos temas."
        }
    ...
    override fun observeRecentSessions(limit: Int): Flow<List<StudySessionEntity>> {
        return studySessionDao.getRecentSessions(limit)
    }

    override fun observeCompletedSessionCount(): Flow<Int> {
        return studySessionDao.observeCompletedSessionCount()
    }
    ...
    override suspend fun getFrequentErrorTypes(limit: Int): List<Pair<String, Int>> {
        return userMasteryDao.getGlobalErrorTypeCounts(limit)
            .map { it.errorType to it.count }
    }
